...
albumArt.addEventListener("change", function () {
  ...
  albumArtDisplay.src = `img/${selectedArt}`; // Set the album art display source to the selected album art
});
...
albumForm.addEventListener("submit", function (event) {
  ...
  const isAlbumTitleInvalid =
    albumTitle.value === "" || albumTitle.value.length > 20; // Check if the album title is invalid
  const isAlbumDescriptionInvalid =
    albumDescription.value === "" || albumDescription.value.length > 40; // Check if the album description is invalid
...
function createAlbumCard(title, description, albumArt) {
  ...
  const albumContainer = document.getElementById("all-album-list");
